...
export function InputAuthors({ value, onchange, label }: React.InputHTMLAttributes<HTMLInputElement> & InputAuthorsProps) {
    // Si value está vacío, iniciamos con un input vacío
    const authorsArray = value ? value.split(",").map((a) => a.trim()) : [""]; // siempre al menos un input
    ...
    return (
        <div className="flex flex-col gap-2">
            <label className="text-primary text-sm font-medium dark:text-gray-400" htmlFor={"author-0"}>
                {label}
            </label>
                ...
                {authorsArray.map((author, index) => (
                    <div key={index} className="flex items-center gap-2">
                        <input
                            ...
                            className="bg-background text-primary-dark w-full flex-1 rounded-xl border px-3 py-2 focus:outline-blue-600 dark:border-white/10 dark:bg-white/5 dark:text-white"
            ...
            <input type="hidden" id={"authors"} value={authorsArray.join(",")} name="authors" />
        </div>
    );
}
